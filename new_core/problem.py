...
class Problem:
	def __init__(self):
		...
		self.dt = 0.0001
		self.tsim = 1
		self.output_spikes = np.empty((10000,2000))
		self.stim = None
		#%% >>>>>>>>> lif variables
...
np.full(400, 2e-10)])
	...
	def integrate_CONN_1(self):
		# E ---NMDA---> E
		self.I_NMDA[0:1600] = (self.gW[1][0]*(self.V[0:1600]-self.VE[0:1600])/(1 + (0.001*np.exp(-0.062*self.V[0:1600]))/3.57))*np.einsum('j,ij->i', self.s_NMDA[0:1600], self.gW[1][1])
	...
	def integrate_CONN_3(self):
		# E ---NMDA---> I
		self.I_NMDA[1600:2000] = (self.gW[3][0]*(self.V[1600:2000]-self.VE[1600:2000])/(1 + (0.001*np.exp(-0.062*self.V[1600:2000]))/3.57))*np.einsum('j,ij->i', self.s_NMDA[0:1600], self.gW[3][1])
	...
	def forward(self, time_idx):
		...
		self.x_NMDA_hist.append(np.copy(self.x_NMDA))  
		self.s_NMDA = self.s_NMDA + ((-self.s_NMDA / 0.1) + 500.0*self.x_NMDA*(1-self.s_NMDA))*self.dt 
		self.s_NMDA_hist.append(np.copy(self.s_NMDA))  
		...
		self.I_syn = self.I_syn + self.I_NMDA/3.57 
		self.I_NMDA_hist.append(np.copy(self.I_NMDA))  
