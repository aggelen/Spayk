...
'N': np.zeros(1),
    ...
    def integrate_synapses(self):
        self.s_AMPA_EXT__CONN_0 += self.d_s_AMPA_EXT__CONN_0*self.dt
    ...
    def calculate_dxdt_all(self):
        ...
        self.d_s_AMPA_EXT__CONN_0 = self.d_s_AMPA_EXT__CONN_0 + (-self.s_AMPA_EXT__CONN_0 / 0.002)+ np.tile(self.current_stimuli['external_stim'], (1,1)) 
        if np.sum(self.d_s_AMPA_EXT__CONN_0):
            aykut = 5
        self.dsAmpa_hist.append(self.d_s_AMPA_EXT__CONN_0)
    #%% synaptic current calculation func
    def calculate_synaptic_currents(self):
        self.I_syn = -5.009999999999998e-10*np.ones(1)
    #%% firing control
